...
- **禁用必须同时做到「功能完全禁用」和「资源零占用」**：不留可用入口与生效路径，也不新增进程/线程/协程、定时器、轮询、重连、订阅或常驻缓存，不忙等、不空转，也不让别处空转。
- **从源头禁用，不要在半路拦截**：优先级依次是「不构造」（服务装配/对象构造处跳过，如 `IBotsService` 及其远端桥接）→「不注册」（常驻监听、`setInterval`、广播/IPC 事件订阅）→「不进入」（UI 触发、IPC handler、命令、启动恢复在源头 return，打开弹窗/向导也算进入）→「半路拦截」仅作纵深防御，不能只做这一层。禁用判定必须用编译期常量，不得用超时兜底。完整判据见 `spec/vendor-disable.md` 的「功能与资源」一节。
