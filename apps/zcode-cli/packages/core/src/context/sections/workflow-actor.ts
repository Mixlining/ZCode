...
//
// 它替换的是交互式的 Agent Identity（identity.ts 的开场句）：子代理的读者是脚本，不是人。
// 它**不**替换基座的其他段——`# Harness` 块从 identity 逐字复用，memory / skills / 项目指令
// 由 builder 照常追加。原来还有一段与交互式身份共用的安全 IMPORTANT 段，已按配置移除，两处
// 身份段都不再注入它。
...
function buildWorkflowActorIdentityPrompt(actor: WorkflowActorContext): string {
  ...
  // 不再有 CLI prefix 走在前面（「You are a helpful software engineer assistant named ZCode」
