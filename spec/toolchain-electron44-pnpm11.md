...
- Keep GitHub Actions on the same Node and pnpm pins by loading `mise.toml`; cache the root pnpm store from `pnpm-lock.yaml` plus the `mise.toml` pin that selects the store layout, and cache Electron binaries from the Desktop manifest. Windows cache entries are owned by the default-branch warm workflow; the packaging workflow only restores them, per `spec/desktop-main-static-contracts.md`. The Windows packaging workflow downloads Electron and electron-builder binaries directly from their official GitHub Releases base URLs.
...
8. GitHub Actions selects Node and pnpm from the repository pins, restores caches keyed by the changed root lockfile, the `mise.toml` toolchain pin, and the Desktop Electron manifest, and uses official npm for packages plus the official GitHub Releases for Electron packaging binaries. Windows cache entries are written on the default branch only, so every release tag can read them.
